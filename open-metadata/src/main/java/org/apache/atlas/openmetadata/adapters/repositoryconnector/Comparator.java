...
public class Comparator {
    ...
    private boolean compare(boolean strict, TypeDef a,TypeDef b) {
        ...
        if (!(compare((TypeDefSummary)a,(TypeDefSummary)b))) {
            LOG.debug("Compare TypeDef: TypeDefSumary did not match");
            return false;
        }
        return true;
    }
    ...
    // This method enables us to compare arryas of heterogeneous def types - e.g. a mixture of ClassificationDef, EntityDef, etc..
    ...
    // This method enables us to compare arraays of attribute type defs
    ...
    private boolean compareConcreteAttributeTypeDefs(boolean strict, AttributeTypeDef a, AttributeTypeDef b) {
        ...
        if (!(aClass.equals(bClass))) {
            LOG.debug("ConcreteAttrbuteTypeDef classes different {} vs {}", aClass, bClass);
            return false;
        }
    ...
    // strict is only used to disable debug logging where we are perfoming non-strict compares
    ...
    public boolean compare(boolean strict, RelationshipDef a, RelationshipDef b) {
        ...
        // The folllowing code is preserved in case relationship category and containership are re-introduced.
...
//    }
     ...
     *   cardinalities,
     *   relationship cateogry - e.g. association vs composition
     *   version??
     *   versionName??
     *
    ...
    public boolean equivalent(TypeDef def1, TypeDef def2) {
        ...
        // Delagate up to Compare for TypeDefSummary (everything at that level and above must match).
