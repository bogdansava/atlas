...
public class LocalAtlasOMRSMetadataCollection extends OMRSMetadataCollection {
    ...
    /**
     * Declare the Atlas stores the connector will use - these are injected via AtlasServicesProxy.
     */
            ...
            UserNotAuthorizedException
    {
         ...
         *
         * The above types can (and probably will) contain atrribute defs. For each attribute the MDC needs to decide what type
         ...
         * In the last two cases (involving type refs) the enclosing typedef will be skipped (not added to the TDG). This is
         ...
         *
         *  Whenever an enclosing typedef (EntityDef, RelationshipDef or ClassificationDef) is added to the TDG it will have a
         * propertiesDef which is a list of TDA, each having the attribute name and an AttributeTypeDef which is on the TDG's
         ...
         * TypeDef a similar linkage is established through a TypeDefLink that refers to a TypeDef on the TDG's typeDefs list.
         * For attribute types that are either consistent and known or valid and new we add them to the TDG's list of attributeTypeDefs.
         ...
         * TDG. References to types in the TDG are by TypeDefLink (which needs a cat, name and guid).
         *
         ...
         * GUID directly. ] I think it is very unlikely that there will be new Prims discovered. In fact I think that is impossible.
         ...
         * If the collection is a known type then we will use the GUID from the RCM and add it to the TDG. It is likely that the RCM
         * will already know about a small number of collections - eg. Map<String,String> - so for these the MDC will reuse the
         * existing attributedef - it will still add the coll def to the TDG and will refer to it usin ga TDL with the cat, anem and guid.
         ...
         * check in the RCM. If known we will do a deep compare and on match will use the existing def, on mismatch genearte audit log entry and
         ...
         * 1. Just assume (assert?) that each type does not exist in the RCM and hence we will project it into the generated TDG. This is a good way
         * to see and verify the generation of types by the MDC. The generator and comparator should be able to generate/compare TDGs.
        ...
        try {
            ...
            e.printStackTrace();
            LOG.error("getAllTypes: caught exception from Atlas {}", e.getErrorMessage());
            throw e;
        }

        /*
         * typeDefsForAPI will have been loaded with all discovered TypeDefs. It can be converted into a TDG
         * and can be reset on the next API to be called.
         */


        // Convert the typeDefsForAPI to a gallery
        TypeDefGallery tdg = typeDefsForAPI.convertTypeDefsToGallery();
        if (LOG.isDebugEnabled()) {
            LOG.debug("<== {} TypeDefGallery={}", methodName, tdg);
        }
        return tdg;
    }
            ...
            UserNotAuthorizedException
    {
        ...
        // Show the injected typeDefStore and typeRegistry - useful for debug
        LOG.debug("findTypesByName: typeDefStore={}", typeDefStore);
        ...
        try {
            ...
            LOG.error("findTypesByName: caught exception from Atlas type def store {}", e.getMessage());
        ...
        TypeDefGallery tdg = typeDefsForAPI.convertTypeDefsToGallery();
        if (LOG.isDebugEnabled()) {
            LOG.debug("<== findTypesByName(userId={}, name={}): tdg={}", userId, name, tdg);
        }
        return tdg;
    }
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("_findTypeDefsByCategory: caught exception from Atlas type def store {}", e.getMessage());
            ...
            UserNotAuthorizedException
    {
        ...
        if (category == ENUM_DEF) {
            ...
            try {
                ...
                LOG.error("_findAttributeTypeDefsByCategory: caught exception from Atlas type def store {}", e.getMessage());
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("findTypeDefsByProperty: caught exception from Atlas type def store {}", e.getMessage());
        ...
        if (superTypeLink != null) {
            ...
            if (superTypeLink.getName() != null) {
                ...
                try {
                    ...
                    LOG.error("getPropertyNames: caught exception from getTypeDefByName {}", e.getMessage());
    ...
    private boolean externalStandardsSatisfyCriteria(List<ExternalStandardMapping> typeDefExternalMappings, ExternalStandardMapping filterCriteria) {
         ...
         * If there is no filterCriteria.identfier or there is and it matches the ESM passes
         */
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("searchForTypeDefs:caught exception from attempt to retrieve all TypeDefs from Atlas repository");
            OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("getTypeDefByGUID: re-throwing exception from internal method");
            throw e;
        }
            ...
            TypeDefNotKnownException
    {
        ...
        try {
            ...
            if (e.getAtlasErrorCode() == AtlasErrorCode.TYPE_GUID_NOT_FOUND) {
                LOG.error("_getTypeDefByName: Atlas does not have the type with guid {} ", guid);
        ...
        try {
            ...
            LOG.error("_getTypeDefByGUID: Failed to convert the Atlas type {} to an OM TypeDef", abtd.getName());
            OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage("guid", "_getTypeDefByGuid",metadataCollectionId);
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("getAttributeTypeDefByGUID: re-throwing exception from internal method");
            throw e;
        }
...

    {
        ...
        try {
            ...
            if (e.getAtlasErrorCode() == AtlasErrorCode.TYPE_GUID_NOT_FOUND) {
                LOG.error("_getAttributeTypeDefByGUID: Atlas does not have the type with guid {} ", guid);
                // The AttributeTypeDef was not found - return null
                return null;
            }
            else {
                LOG.error("_getAttributeTypeDefByGUID: caught exception trying to retrieve type with guid {}", guid);
                OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
                String errorMessage = errorCode.getErrorMessageId()
                        + errorCode.getFormattedErrorMessage("name", "_getAttributeTypeDefByGUID", metadataCollectionId);
        ...
        if (abtd != null) {
            ...
            LOG.error("_getAttributeTypeDefByGUID: received null return from Atlas getByGuid using guid {}", guid);
            OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage("name", "_getAttributeTypeDefByGUID",metadataCollectionId);
        ...
        if (candidateAttributeTypeDef == null) {
            ...
            try {
                ...
                LOG.error("_getAttributeTypeDefByGUID: Caught exception from repository helper for attribute type def with name {}", name);
                OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
                String errorMessage = errorCode.getErrorMessageId()
                        + errorCode.getFormattedErrorMessage("name", "_getAttributeTypeDefByGUID",metadataCollectionId);
            ...
            if (existingAttributeTypeDef == null) {
                ...
                if (typematch) {
                    // We will add the attributetypedef to the TDG
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("getTypeDefByName: re-throwing exception from internal method");
            throw e;
        }
            ...
            TypeDefNotKnownException
    {
        ...
        try {
            ...
            if (e.getAtlasErrorCode() == AtlasErrorCode.TYPE_NAME_NOT_FOUND) {
                ...
                LOG.error("_getTypeDefByName: Caught exception trying to retrieve Atlas type with name {} ", atlasTypeName);
                OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
                String errorMessage = errorCode.getErrorMessageId()
        ...
        try {
            ...
            LOG.error("_getTypeDefByName: Failed to convert the Atlas type {} to an OM TypeDef", abtd.getName());
            OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("getAttributeTypeDefByName: re-throwing exception from internal method");
            throw e;
        }
       ...
       if (name == null) {
        ...
        try {
            ...
            if (e.getAtlasErrorCode() == AtlasErrorCode.TYPE_NAME_NOT_FOUND) {
                ...
                LOG.error("_getAttributeTypeDefByName: Caught exception trying to retrieve Atlas type with name {} ", name);
                OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
                String errorMessage = errorCode.getErrorMessageId()
                        + errorCode.getFormattedErrorMessage("name", "_getAttributeTypeDefByName", metadataCollectionId);
        ...
        if (abtd != null) {
            ...
            LOG.error("_getAttributeTypeDefByName: received null return from Atlas getByName using name {}", name);
            OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage("name", "_getAttributeTypeDefByName", metadataCollectionId);
        ...
        if (candidateAttributeTypeDef == null) {
            ...
            try {
                ...
                LOG.error("_getAttributeTypeDefByName: caught exception from repository helper for type name {}", name);
                OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
                String errorMessage = errorCode.getErrorMessageId()
                        + errorCode.getFormattedErrorMessage("name", "_getAttributeTypeDefByName", metadataCollectionId);
    ...
    public void addTypeDef(String  userId,
         ...
         *  Create an empty typesdef container for type to be converted below
         */
        ...
        try {
            ...
            LOG.error("addTypeDef: exception from store and retrieve AtlasTypesDef {}", e.getMessage());
            e.printStackTrace();
            OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage("TypeDef", "retrieval", metadataCollectionId);
        ...
        if (fatalError || returnableTypeDef == null) {
            ...
            LOG.error("addTypeDef: could not intialise mapper or convert retrieved AtlasBaseTypeDef {} to OM TypeDef", newTypeName);
            OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
            ...
            UserNotAuthorizedException
    {
        ...
        // Create an empty typesdef container for whatever we convert below..
        AtlasTypesDef atlasTypesDef = new AtlasTypesDef();
        ...
        if (category == ENUM_DEF) {
            ...
            try {
                ...
                LOG.error("addAttributeTypeDef: caught exception from Atlas, error code {}",e.getAtlasErrorCode());
                e.printStackTrace();
            ...
            UserNotAuthorizedException
    {
        ...
        switch (atdCat) {
            ...
            case PRIMITIVE:
                LOG.debug("verifyAttributeTypeDef: supplied AttributeTypeDef has cateogory PRIMITIVE - returning false");
                ret = false;
                break;

            case COLLECTION:
                LOG.debug("verifyAttributeTypeDef: supplied AttributeTypeDef has cateogory COLLECTION - returning false");
                ret = false;
                break;

            case ENUM_DEF:
                boolean conflict = false;
                LOG.debug("verifyAttributeTypeDef: supplied AttributeTypeDef has cateogory ENUM_DEF - checking for existence");
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("updateTypeDef: The repository could not find the TypeDef");
            OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage("typeDefGUID or typeName", methodName, repositoryName);
        ...
        if (fatalError || omPreTypeDef == null) {
            ...
            LOG.error("updateTypeDef: could not intialise mapper or convert retrieved AtlasBaseTypeDef {} to OM TypeDef", atlasPreTypeDef.getName());
            OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
        ...
        try {
            ...
            LOG.error("updateTypeDef: exception from store and retrieve AtlasTypesDef {}", e.getMessage());
            e.printStackTrace();
        ...
        if (fatalError || returnableTypeDef == null) {
            ...
            LOG.error("updateTypeDef: could not intialise mapper or convert retrieved AtlasBaseTypeDef {} to OM TypeDef", typeName);
            OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage("TypeDef", "updateTypeDef", metadataCollectionId);
            throw new RepositoryErrorException(errorCode.getHTTPErrorCode(),
            ...
            UserNotAuthorizedException
    {
         ...
         */
            ...
            LOG.error("The retrieved TypeDef could not be converted to Atlas format");
            OMRSErrorCode errorCode = OMRSErrorCode.INVALID_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage(obsoleteTypeDefName, obsoleteTypeDefGUID, guidParameterName, methodName, repositoryName, omTypeDefToDelete.toString());
            ...
            LOG.error("The Atlas repository could not delete the TypeDef");
            OMRSErrorCode errorCode = OMRSErrorCode.INVALID_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage(obsoleteTypeDefName, obsoleteTypeDefGUID, guidParameterName, methodName, repositoryName, omTypeDefToDelete.toString());
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            try {
                ...
                LOG.error("deleteAttributeTypeDef: caught exception trying to retrieve type with guid {}", obsoleteTypeDefGUID);
                OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
                String errorMessage = errorCode.getErrorMessageId()
                        + errorCode.getFormattedErrorMessage("guid", "deleteAttributeTypeDef",metadataCollectionId);
            ...
            LOG.error("The Atlas repository could not delete the AttributeTypeDef");
            OMRSErrorCode errorCode = OMRSErrorCode.INVALID_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage(obsoleteTypeDefName, obsoleteTypeDefGUID, guidParameterName, methodName, repositoryName, abtd.toString());
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("isEntityKnown: caught EntityNotKnownException exception from getEntityDetail - exception swallowed, returning null");
            return null;
        } catch (RepositoryErrorException e) {
            LOG.error("isEntityKnown: caught RepositoryErrorException exception from getEntityDetail - rethrowing");
            throw e;
        } catch (UserNotAuthorizedException e) {
            LOG.error("isEntityKnown: caught UserNotAuthorizedException exception from getEntityDetail - rethrowing");
            throw e;
        }
    ...
    public boolean isEntityProxy(String guid) throws EntityNotKnownException {
        ...
        try {
            ...
            LOG.error("isEntityProxy: caught exception from to get entity from Atlas repository, guid {}", guid);
    ...
    public boolean isEntityLocal(String guid) throws EntityNotKnownException {
        ...
        try {
            ...
            LOG.error("isEntityLocal: caught exception from to get entity from Atlas repository, guid {}", guid);
            OMRSErrorCode errorCode = OMRSErrorCode.ENTITY_NOT_KNOWN;
            String errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage(guid, methodName, metadataCollectionId);
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("getEntitySummary: caught exception from to get entity from Atlas repository, guid {}", guid);
            OMRSErrorCode errorCode = OMRSErrorCode.ENTITY_NOT_KNOWN;
            String errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage(guid, methodName, metadataCollectionId);
        ...
        try {
            ...
            LOG.error("getEntitySummary: caught exception {}",e.getMessage());
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("getEntityDetail: caught exception from get entity by guid {}, {}",guid, e.getMessage());
        ...
        try {
            ...
            LOG.error("getEntityDetail: caught exception from attempt to convert Atlas entity to OM {}, {}",atlasEntity, e.getMessage());
...

    {
        ...
        // Seqeuenced queries are not yet supported.
        ...
        // Paged queries are not yet supported.
        if (pageSize != 0) {
            LOG.debug("getRelationshipsForEntity: Request to page relationships uwing pageSize {}", pageSize);
            OMRSErrorCode errorCode = OMRSErrorCode.METHOD_NOT_IMPLEMENTED;
            String errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage(entityGUID, methodName, metadataCollectionId);
        ...
        // Result offset is not yet supported.
        ...
        try {
            ...
            LOG.error("getRelationshipsForEntity: caught exception from attempt to get Atlas entity by GUID {}",entityGUID);
        ...
        try {
            ...
            if (allRelationships != null && !allRelationships.isEmpty() ) {
                for (Relationship r : allRelationships) {
                     ...
                     *  consider including it in the reault.
                     */
            ...
            LOG.error("getRelationshipsForEntity: Caught exception from AtlasEntityMapper {}", e.getMessage());
            ...
            UserNotAuthorizedException
    {
        ...
        // Paged queries are not yet supported.
        if (pageSize != 0) {
            LOG.debug("findEntitiesByProperty: Request to page entities uwing pageSize {}", pageSize);
            OMRSErrorCode errorCode = OMRSErrorCode.METHOD_NOT_IMPLEMENTED;
            String errorMessage = errorCode.getErrorMessageId()
        ...
        if (entityTypeGUID != null) {   // Use DSL
            try {
                returnList = findEntitiesByPropertyUsingDSL(userId, entityTypeGUID, matchProperties, matchCriteria, limitResultsByStatus, limitResultsByClassification);
            }
            ...
            returnList = findEntitiesByPropertyUsingSearchParameters(userId, matchProperties, matchCriteria,  limitResultsByStatus, limitResultsByClassification);
                ...
                UserNotAuthorizedException
    {
        ...
        if (entityTypeGUID != null)
        {
            ...
            try {
                ...
                // handle below
                LOG.error("findEntitiesByClassification: caught exception from _getTypeDefByGUID {}", e.getMessage());
                entityTypeDef = null;
            }
        ...
        // Sequencing of results is not yet supported.
        if (!StringUtil.isBlank(sequencingProperty)) {
        ...
        // Paging of results is not yet supported.
        if (pageSize != 0) {
        ...
        if (entityTypeGUID != null) {
            // Use DSL
            returnList = findEntitiesByClassificationUsingDSL(userId, entityTypeGUID, classificationName, matchClassificationProperties, matchCriteria,  limitResultsByStatus);

        } else {
            // Type is not specified so use searchWithParameters
            returnList = findEntitiesByClassificationUsingSearchParameters(userId, classificationName, matchClassificationProperties, matchCriteria,  limitResultsByStatus);
        }
...

    {
        ...
        // Paging of results not yet supported.
        if (pageSize != 0) {
        ...
        if (entityTypeGUID != null) {
            ...
            try {
                returnList = findEntitiesByPropertyUsingDSL(userId, entityTypeGUID, searchCriteria, limitResultsByStatus, limitResultsByClassification);
            }
            catch (TypeErrorException | RepositoryErrorException | PropertyErrorException e) {
                LOG.error("findEntitiesByPropertyValue: Caught exception from findEntitiesByPropertyUsingDSL");
                OMRSErrorCode errorCode = OMRSErrorCode.ENTITY_NOT_KNOWN;
                String errorMessage = errorCode.getErrorMessageId()
            ...
            returnList = findEntitiesByPropertyUsingSearchParameters(userId, searchCriteria, limitResultsByStatus, limitResultsByClassification);
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("getRelationship: Caught exception from Atlas {}", e.getMessage());
            ...
            UserNotAuthorizedException
    {
        ...
        if (initialClassifications != null) {
            for (Classification classification :  initialClassifications ) {
                ...
                try {
                    ...
                    LOG.error("addEntity: Could not find classification def with name {}", classification.getName());
                ...
                if (validEntityDefs != null) {
                    // This assumes thtat if validEntityDefs is null then the classification can be attached (to anything).
                    boolean match = false;
        ...
        try {
            ...
            LOG.error("addEntity: Caught exception from Atlas {}", e.getMessage());
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("addEntityProxy: Caught exception trying to create entity proxy, {} ", e.getMessage());
        ...
        if (entityProxy.getClassifications() != null && entityProxy.getClassifications().size() > 0) {
            ...
            try {
                ...
                // Failed to add classifications to the entity
                LOG.error("addEntityProxy: Atlas created entity, but we could not add classifications to it, guid {}", entityProxy.getGUID());
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.debug("udpateEntityStatus: atlasEntity to update is {}", atlasEntity);
            ...
            if (atlasEntWithExt == null) {
                LOG.error("udpateEntityStatus: Could not find entity with guid {} ", entityGUID);
                OMRSErrorCode errorCode    = OMRSErrorCode.ENTITY_NOT_KNOWN;
                String        errorMessage = errorCode.getErrorMessageId()
            ...
            LOG.error("udpateEntityStatus: Caught exception from AtlasEntityMapper {}", e.getMessage());
            ...
            LOG.error("updateEntityStatus: Caught exception from Atlas {}", e.getMessage());
            OMRSErrorCode errorCode    = OMRSErrorCode.ENTITY_NOT_KNOWN;
            String        errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage(entityGUID, entityGUIDParameterName, methodName, repositoryName);
            ...
            UserNotAuthorizedException
    {
        ...
        if (atlasDeleteConfiguration == AtlasDeleteOption.HARD) {
            LOG.error("deleteEntity: Repository is configured for hard deletes, cannnot soft delete entity with guid {}",
                    obsoleteEntityGUID);
            ...
            UserNotAuthorizedException
    {
        ...
        //catch (AtlasBaseException e) { // TODO - catch appropriate exceptions when ATLAS-2774 is available
            ...
            LOG.error("purgeEntity: Caught exception from Atlas entityStore trying to purge entity {}", deletedEntityGUID);
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("classifyEntity: Caught exception from AtlasEntityMapper {}", e.getMessage());
        ...
        try {
            ...
            LOG.error("classifyEntity: caught exception {}", e.getMessage());
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("declassifyEntity: Caught exception from Atlas {}", e.getMessage());
            // handle below
            atlasEntWithExt = null;
        }
        ...
        List<AtlasClassification> preClassifications = atlasEntity.getClassifications();
        // Find the classification to remove...
        if (preClassifications == null) {
            LOG.error("declassifyEntity: Entity with guid {} has no classifications", entityGUID);
            OMRSErrorCode errorCode    = OMRSErrorCode.ENTITY_NOT_CLASSIFIED;
            String        errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage(entityGUID, entityGUIDParameterName, methodName, repositoryName);
        ...
        for (AtlasClassification classification : preClassifications ) {
            if (classification.getTypeName().equals(classificationName)) {
                preClassifications.remove(classification);
            }
        }
        atlasEntity.setClassifications(preClassifications);

        LOG.debug("declassifyEntity: atlasEntity now has classifications {}", atlasEntity.getClassifications());
        ...
        try {
            ...
            LOG.error("declassifyEntity: Could not convert Atlas entity with guid {} ", entityGUID);
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("updateEntityClassification: Caught exception from Atlas {}", e.getMessage());
            // handle below
            atlasEntWithExt = null;
        }
        ...
        try {
            ...
            LOG.error("updateEntityClassification: Caught exception from AtlasEntityMapper {}", e.getMessage());
        ...
        LOG.debug("updateEntityClassification: atlasEntity now has classifications {}", atlasEntity.getClassifications());
        ...
        try {
            ...
            LOG.error("updateEntityClassification: Caught exception from AtlasEntityMapper, entity guid {}, {} ", entityGUID, e.getMessage());
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("addRelationship: Caught exception from Atlas {}", e.getMessage());
            // Handle below
        ...
        try {
            ...
            LOG.error("addRelationship: caught exception from AtlasEntityMapper {}", e.getMessage());
        ...
        try {
            ...
            LOG.error("addRelationship: caught exception {}", e.getMessage());
        ...
        try {
            ...
            LOG.error("addRelationship: caught exception from relationship store create method {}", e.getMessage());
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("updateRelationshipStatus: Caught exception from Atlas {}", e.getMessage());
        ...
        try {
            ...
            LOG.error("updateRelationshipStatus: Caught exception from Atlas relationship store update method {}", e.getMessage());
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("updateRelationshipProperties: Caught exception from Atlas {}", e.getMessage());
        ...
        try {
            ...
            LOG.error("updateRelationshipProperties: Caught exception from Atlas relationship store create method, {}", e.getMessage());
            // handle below
            ...
            UserNotAuthorizedException
    {
        ...
        // One final piece of validation - if deletes are configured as HARD then throw not supported exception
        if (atlasDeleteConfiguration == AtlasDeleteOption.HARD) {
            LOG.error("deleteRelationship: Repository is configured for hard deletes, cannnot soft delete relationship with guid {}",
            ...
            UserNotAuthorizedException
    {
        ...
        //catch (AtlasBaseException e) { // TODO - catch appropriate exceptions when ATLAS-2774 is available
            ...
            LOG.error("purgeRelationship: Caught exception from Atlas relationshipStore trying to purge relationship {}", deletedRelationshipGUID);
...

    {
        ...
        try {
            ...
            LOG.error("retypeEntity: Caught OMRS exception {}", e.getMessage());
            ...
            LOG.error("retypeEntity: Caught exception from Atlas {}", e.getMessage());
            OMRSErrorCode errorCode    = OMRSErrorCode.ENTITY_NOT_KNOWN;
            String        errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage(entityGUID, entityParameterName, methodName, repositoryName);
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("retypeEntity: Caught exception from AtlasEntityMapper {}", e.getMessage());
...

    {
        ...
        try {
            ...
            LOG.error("reTypeRelationship: Caught OMRS exception {}", e.getMessage());
            OMRSErrorCode errorCode    = OMRSErrorCode.RELATIONSHIP_NOT_KNOWN;
            String        errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage(relationshipGUID, relationshipParameterName, methodName, repositoryName);
            ...
            LOG.error("reTypeRelationship: Caught exception from Atlas {}", e.getMessage());
            OMRSErrorCode errorCode    = OMRSErrorCode.RELATIONSHIP_NOT_KNOWN;
            String        errorMessage = errorCode.getErrorMessageId()
                    + errorCode.getFormattedErrorMessage(relationshipGUID, relationshipParameterName, methodName, repositoryName);
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            LOG.error("saveEntityReferenceCopy: Could not set entity status for entity with GUID {} ", entity.getGUID());
            OMRSErrorCode errorCode = OMRSErrorCode.BAD_INSTANCE_STATUS;
            String errorMessage = errorCode.getErrorMessageId() + errorCode.getFormattedErrorMessage(methodName,
                    this.getClass().getName(),
                    repositoryName);
        ...
        try {
            ...
            LOG.error("saveEntityReferenceCopy: Caught exception trying to create entity, {} ", e.getMessage());
            OMRSErrorCode errorCode = OMRSErrorCode.ENTITY_NOT_KNOWN;
            String errorMessage = errorCode.getErrorMessageId() + errorCode.getFormattedErrorMessage(methodName,
                    this.getClass().getName(),
                    repositoryName);
        ...
        if (entity.getClassifications() != null && entity.getClassifications().size() > 0) {
            ...
            try {
                ...
                LOG.error("saveEntityReferenceCopy: Atlas saved entity, but could not add classifications to it, guid {}", entity.getGUID());
                OMRSErrorCode errorCode = OMRSErrorCode.INVALID_CLASSIFICATION_FOR_ENTITY;
                String errorMessage = errorCode.getErrorMessageId() + errorCode.getFormattedErrorMessage(methodName,
                        this.getClass().getName(),
                        repositoryName);
            ...
            UserNotAuthorizedException
    {
        ...
        try {
            ...
            try {
                ...
                LOG.error("saveRelationshipReferenceCopy: caught exception from addEntityProxy {}", exc.getMessage() );
                LocalAtlasOMRSErrorCode errorCode = LocalAtlasOMRSErrorCode.ENTITY_NOT_CREATED;
        ...
        // Create AtlasRelationship
        // GUID is set by Atlas CTOR to nextInternalID - but because we are saving a ref copy we overwrite with the provided GUID
        AtlasRelationship atlasRelationship = new AtlasRelationship();
        ...
        //
        atlasRelationship.setGuid(relationship.getGUID());
        ...
        try {
            ...
            LOG.debug("saveRelationshipReferenceCopy: Caught exception from Atlas relationship store create method {}", e.getMessage());
            ...
            TypeErrorException
    {
        ...
        try {
            ...
            LOG.error("convertOMRelationshipDef: could not create an AtlasRelationshipDef for type {}",omRelationshipDef.getName());
            OMRSErrorCode errorCode = OMRSErrorCode.NO_TYPEDEF;
            String errorMessage = errorCode.getErrorMessageId()
         ...
         * for a Glossary that has a TermAnchor relationship to a Term will be stored as type="Glossary";attribName="terms".
         * This is the opposite to OM which defines the RelEndDef for Glossary as type="Glossary"; attribName="anchor" - ie.
         ...
         * checking or comparison - e.g. that the entiti defs exist. That is delegated to Atlas since it has that
         * checking already.
         */
         ...
         */
        RelationshipEndCardinality omEndDef1Card = omEndDef2.getAttributeCardinality(); // attribute cardinalities are deliberately transposed as commented above
         ...
         * instances as a one-to-many, many-to-one, many-to-many.
         */
        RelationshipEndCardinality omEndDef2Card = omEndDef1.getAttributeCardinality();  // attribute cardinalities are deliberately transposed as commented a
    ...
    // defs encountered in the Atlas typesdef
    //
        ...
        if (atlasEnumDefs != null) {
            for (AtlasEnumDef atlasEnumDef : atlasEnumDefs) {
                try {
                    ...
                    LOG.error("processAtlasEnumDefs: gave up on AtlasEnumDef {}", atlasEnumDef.getName());
        ...
        if (atlasEntityDefs != null) {
            for (AtlasEntityDef atlasEntityDef : atlasEntityDefs) {
                try {
                    ...
                    LOG.error("processAtlasEntityDefs: gave up on AtlasEntityDef {}",atlasEntityDef.getName());
                    // swallow the exception to proceed with the next type...
                }
            }
        }
        ...
        if (atlasClassificationDefs != null) {
            for (AtlasClassificationDef atlasClassificationDef : atlasClassificationDefs) {
                try {
                    ...
                    LOG.error("processAtlasClassificationDefs: gave up on AtlasClassificationDef {}",atlasClassificationDef.getName());
                    // swallow the exception to proceed with the next type...
                }
        ...
        if (atlasRelationshipDefs != null) {
            for (AtlasRelationshipDef atlasRelationshipDef : atlasRelationshipDefs) {
                try {
                    ...
                    LOG.error("processAtlasRelationshipDefs: gave up on AtlasRelationshipDef {}",atlasRelationshipDef.getName());
                    // swallow the exception to proceed with the next type...
                }
    ...
    //       i.  If same use the existing type's GUID; add def to TDG;
    //       ii. If different fail (audit log)
    //    b. If not exists (in RCM), generate a GUID and add to TDG.
    //
            ...
            TypeErrorException
    {
        ...
        if (existingAttributeTypeDef == null) {
            ...
            // If it does not already exist add the new ATD to the ATDs in the TDG and use it in TDA.
            ...
            // Add the OM typedef to TDBC - this will get it into the TDG
            ...
            if (existingAttributeTypeDef.getCategory() == ENUM_DEF) {
                ...
                if (typematch) {
                    // There is exact match in the ReposHelper - we will add that to our TDG
            ...
            RepositoryErrorException
    {
        ...
        try {
            ...
            LOG.error("processAtlasClassificationDef: could not initialise mapper");
            throw e;
        }
        catch (TypeErrorException e) {
            LOG.error("processAtlasClassificationDef: could not convert AtlasEntityDef {} to OM EntityDef", typeName);
            throw e;
        }
        ...
        if (existingTypeDef == null) {
            ...
            if (existingTypeDef.getCategory() == CLASSIFICATION_DEF) {
                ...
                // If compare matches use the known type
                if (typematch) {
                    // We will add the typedef to the TDG
        ...
        // Add the OM typedef to discoveredTypeDefs - this will get it into the TDG
        LOG.debug("convertClassificationDef: OMRS ClassificationDef {}", classificationDef);
        // Add the OM typedef to TDBC - this will ultimately get it into the TDG
            ...
            throws TypeErrorException, RepositoryErrorException
    {
        ...
        try {
            ...
            LOG.error("processAtlasEntityDef: could not intialise mapper");
            throw e;
        }
        catch (TypeErrorException e) {
            LOG.error("processAtlasEntityDef: could not convert AtlasEntityDef {} to OM EntityDef", typeName);
            throw e;
        }
            ...
            throws TypeErrorException, RepositoryErrorException
    {
        ...
        try {
            ...
            LOG.error("processAtlasClassificationDef: could not intialise mapper");
            throw e;
        }
        catch (TypeErrorException e) {
            LOG.error("processAtlasClassificationDef: could not convert AtlasEntityDef {} to OM EntityDef", typeName);
        ...
        try {
            ...
            e.printStackTrace();
            // Fail the conversion by returning without adding the def to the TDG
    ...
    private AtlasStructDef.AtlasAttributeDef convertOMCollectionDef(TypeDefAttribute tda) {
        ...
        switch (collDefCat) {
            ...
            case OM_COLLECTION_MAP:
                ...
                break;
            default:
                LOG.debug("convertOMCollectionDef: cannot onvert a collection def with category {}", collDefCat);
                return null;
        }
            ...
            throws RepositoryErrorException
    {
        ...
        try {
            ...
            LOG.error("loadAtlasTypeDefs: caught exception from Atlas searchTypesDef {}",e.getMessage());
                ...
                while (propNames.hasNext()) {
                    ...
                    switch (ipvCat) {
                        ...
                        case ENUM:
                            ...
                            Object eunmValue = enumPropertyValue.getSymbolicName();
                            atlasAttrs.put(propName, eunmValue);
                            break;
                        case STRUCT:
                        case UNKNOWN:
                        default:
            ...
            throws TypeErrorException, RepositoryErrorException, PropertyErrorException
    {
        ...
        try {
            ...
            // Handle below
            LOG.error("findEntitiesByPropertyUsingDSL: Caught exception from _getTypeDefByGUID {}", e.getMessage());
            typeDef = null;
        }
        ...
        try {
            ...
            retList = findEntitiesByPropertyUsingDSL(userId, entityTypeGUID, matchStringProperties,
                    MatchCriteria.ANY, limitResultsByStatus, limitResultsByClassification);

        } catch (TypeDefNotKnownException e) {

            LOG.error("findEntitiesByPropertyUsingDSL: Could not retrieve properties of entity type with GUID {}", entityTypeGUID);
...

    {
        ...
        try {
            ...
            e.printStackTrace();
            LOG.error("findEntitiesByPropertyUsingDSL: re-throwing exception from createWhereClause");
            throw e;
        ...
        try {
            ...
            LOG.error("findEntitiesByPropertyUsingDSL: Atlas DSL query threw exception {}", e.getMessage());
        ...
        if (atlasEntities != null) {
            ...
            for (AtlasEntityHeader aeh : atlasEntities) {
                ...
                if (limitResultsByStatus != null) {
                    // Need to check that the AEH satus is in the list of allowed status values
                ...
                try {
                    ...
                    LOG.error("findEntitiesByPropertyUsingDSL: caught exception from Atlas entity store getById {}", e.getMessage());
                ...
                try {
                    ...
                    LOG.error("findEntitiesByPropertyUsingDSL: could not map AtlasEntity to EntityDetail {}", e.getMessage());
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("==> findEntitiesByPropertyUsingDSL(userId={}, entityTypeGUID={}, matchProperties={}, matchCriteria={}, limitResultsByStatus={}, limitResultsByClassification={}: returnList={})",
                    userId, entityTypeGUID, matchProperties, matchCriteria, limitResultsByStatus, limitResultsByClassification, returnList);
        }
        return returnList;
    }
            ...
            RepositoryErrorException
    {
        ...
        if (searchCriteria == null || searchCriteria.equals("")) {
            ...
            LOG.debug("findEntitiesByPropertyUsingSearchParameters: no search critera supplied, returning null");
            return null;
        }
        ...
        // TODO - offset and limit should be managed by pageSize etc.
        ...
        try {
            ...
            LOG.error("findEntitiesByPropertyUsingSearchParameters: entity discovery service searchWithParameters threw exception {}", e.getMessage());
        ...
        if (atlasEntities != null) {
            ...
            for (AtlasEntityHeader aeh : atlasEntities) {
                ...
                try {
                    ...
                    LOG.error("findEntitiesByPropertyUsingSearchParameters: entity store getById threw exception {}", e.getMessage());
                ...
                try {
                    ...
                    LOG.error("findEntitiesByPropertyUsingSearchParameters: could not map AtlasEntity to entitDetail, exception {}", e.getMessage());
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("<== findEntitiesByPropertyUsingSearchParameters(userId={}, searchCriteria={}, limitResultsByStatus={}, limitResultsByClassification={}): returnList={}",
                    userId, searchCriteria, limitResultsByStatus, limitResultsByClassification, returnList);
        }
        return returnList;
    }
            ...
            RepositoryErrorException
    {
        ...
        try {
            ...
            LOG.error("findEntitiesByPropertyUsingSearchParameters: caught exception from _findTypeDefsByCategory {}", e.getMessage());
        ...
        if (allEntityTypes == null) {
            LOG.debug("findEntitiesByPropertyUsingSearchParameters: found no entitiy types");
            return null;
        }
        ...
        if (allEntityTypes.size() > 0) {
            ...
            for (TypeDef tdef : allEntityTypes) {
                LOG.debug("findEntitiesByPropertyUsingSearchParameters: checking entity type {}", tdef.getName());

                // If there are any matchProperties
                if (matchProperties != null) {
                    // Validate that the type has the specified properties...
                    // Whethr matchCriteria is ALL | ANY | NONE we need ALL the match properties to be defined in the type definition
                    ...
                    try {
                        ...
                        definedAttributes = getAllDefinedProperties(userId, tdef);

                    }
                    catch (TypeDefNotKnownException e) {

                        LOG.error("findEntitiesByPropertyUsingSearchParameters: caught exception from property finder");
                    ...
                    if (definedAttributes == null) {
                        ...
                        LOG.debug("findEntitiesByPropertyUsingSearchParameters: entity type {} has no properties - will be ignored", tdef.getName());
                        // continue;
                    }
                    else {
                        ...
                        while (matchPropNames.hasNext()) {
                            ...
                            for (TypeDefAttribute defType: definedAttributes) {
                                ...
                                if (defType.getAttributeName().equals(matchPropName) && defTypeName.equals(matchPropType)) {
                                    ...
                                    LOG.debug("findEntitiesByPropertyUsingSearchParameters: entity type {} has property name {} type {}", tdef.getName(),matchPropName,matchPropType);
                            ...
                            if (!propertyDefined) {
                                ...
                                LOG.debug("findEntitiesByPropertyUsingSearchParameters: entity type {} does not have property name {} type {}", tdef.getName(),matchPropName,matchPropType);
                                allPropsDefined = false;
                            }
                        }
                        if (!allPropsDefined) {
                            // At least one property in the match props is not defined on the type - skip the type
                            LOG.debug("findEntitiesByPropertyUsingSearchParameters: entity type {} does not have all match properties - will be ignored", tdef.getName());
                            //continue;
                        }
                        else {
                            // The current type is suitable for a find of instances of this type.
                            LOG.debug("findEntitiesByPropertyUsingSearchParameters: entity type {} will be searched", tdef.getName());

                            // Extract the type guid and invoke a type specific search...
                            String typeDefGUID = tdef.getGUID();

                            ArrayList<EntityDetail> entitiesForCurrentType = findEntitiesByPropertyUsingSearchParameters(
                                    userId, typeDefGUID, matchProperties, matchCriteria, limitResultsByStatus, limitResultsByClassification);

                            if (entitiesForCurrentType != null && !entitiesForCurrentType.isEmpty()) {
                                if (returnEntities == null) {
                                    returnEntities = new ArrayList<>();
                                }
                                LOG.debug("findEntitiesByPropertyUsingSearchParameters: for type {} found {} entities", tdef.getName(),entitiesForCurrentType.size());
                                returnEntities.addAll(entitiesForCurrentType);
                            }
                            else {
                                LOG.debug("findEntitiesByPropertyUsingSearchParameters: for type {} found no entities", tdef.getName());
                            }
                        }
                    }
                }
        ...
        LOG.debug("findEntitiesByPropertyUsingSearchParameters: found {} entities", resultSize);
        if (LOG.isDebugEnabled()) {
            LOG.debug("<== findEntitiesByPropertyUsingSearchParameters(userId={}, matchProperties={}, matchCriteria={}, limitResultsByStatus={}, limitResultsByClassification={}): returnEntities={}",
                    userId, matchProperties, matchCriteria, limitResultsByStatus, limitResultsByClassification, returnEntities);
        }
    ...
    // Utility method for recursing up supertype hierarchy fetching property defs
    ...
    throws RepositoryErrorException, TypeDefNotKnownException
    {
        ...
        if (tdef.getSuperType() != null) {
            ...
            try {
                ...
                LOG.error("getAllDefinedProperties: caught exception from _getTypeDefByName");
                throw e;
            }
        }
            ...
            RepositoryErrorException
    {
        ...
        if (entityTypeGUID != null) {
            ...
            try {
                ...
                LOG.error("findEntitiesByPropertyUsingSearchParameters: caught exception from attempt to look up type by GUID {}", entityTypeGUID);
                return null;
            }
            if (tDef == null) {
        ...
        if (atlasEntities != null) {
            ...
            for (AtlasEntityHeader aeh : atlasEntities) {
                ...
                try {
                    ...
                    LOG.error("findEntitiesByPropertyUsingSearchParameters: could not map AtlasEntity to EntityDetail {}", e.getMessage());
...

    {
        ...
        try {
            ...
            LOG.error("findEntitiesByClassificationUsingDSL: caught exception from _getTypeDefByGUID {}", e.getMessage());
            // handle below
            typeDef = null;
        }
        ...
        try {
            ...
            e.printStackTrace();
            LOG.error("findEntitiesByClassificationUsingDSL: re-throwing exception from createWhereClause");
            throw e;
        ...
        try {
            ...
            LOG.error("findEntitiesByClassificationUsingDSL: Atlas searchUsingDslQuery threw exception {}", e.getMessage());
        ...
        if (atlasEntities != null) {
            ...
            for (AtlasEntityHeader aeh : atlasEntities) {
                ...
                try {
                    ...
                    LOG.error("findEntitiesByClassificationUsingDSL: Atlas entity store getById threw exception {}", e.getMessage());
                ...
                try {
                    ...
                    LOG.error("findEntitiesByClassificationUsingDSL: could not map AtlasEntity to EntityDetail, exception {}", e.getMessage());
            ...
            RepositoryErrorException
    {
        ...
        try {
            ...
            LOG.error("findEntitiesByClassificationUsingSearchParameters: entity discovery service searchWithParameters threw exception {}", e.getMessage());
        ...
        if (atlasEntities != null) {
            ...
            for (AtlasEntityHeader aeh : atlasEntities) {
                ...
                try {
                    ...
                    LOG.error("findEntitiesByClassificationUsingSearchParameters: entity store getById threw exception {}", e.getMessage());

                    OMRSErrorCode errorCode = OMRSErrorCode.ENTITY_NOT_KNOWN;

                    String errorMessage = errorCode.getErrorMessageId()
                            + errorCode.getFormattedErrorMessage(aeh.getGuid(), "entitiy GUID", methodName, metadataCollectionId);
                ...
                try {
                    ...
                    LOG.error("findEntitiesByClassificationUsingSearchParameters: could not map AtlasEntity to EntityDetail, exception {}", e.getMessage());

                    OMRSErrorCode errorCode = OMRSErrorCode.INVALID_ENTITY_FROM_STORE;

                    String errorMessage = errorCode.getErrorMessageId()
                            + errorCode.getFormattedErrorMessage(atlasEntity.getGuid(), "entitiy GUID", methodName, metadataCollectionId);
    ...
    /*
     * Helper method to avoid needing to pass all the store referances to the various mappers
     */
            ...
            TypeErrorException
    {
        ...
        try {
            aadm = new AtlasAttributeDefMapper(this, userId, typeDefStore, repositoryHelper, typeDefsForAPI, aads);
        } catch (RepositoryErrorException e) {
            LOG.error("convertAtlasAttributeDefs: could not create mapper {}", e.getMessage());
            throw e;
        }
